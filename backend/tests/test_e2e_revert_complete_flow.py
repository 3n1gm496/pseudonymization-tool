...
def test_complete_e2e_flow():
    ...
    assert pseudonymized_text != "John Smith from New York", \
        "Text wasn't modified - no pseudonymization happened!"
    ...
    assert mapping_bytes.startswith(b'PSM2'), \
        f"Invalid magic bytes: expected PSM2, got {mapping_bytes[:4].hex()}"
    ...
    assert isinstance(mapping_dict["mapping"], dict), \
        f"Mapping should be dict, got {type(mapping_dict['mapping'])}"
...
async def test_e2e_flow_with_wrong_passphrase():
    ...
    assert revert_resp.status_code == 400, \
        f"Expected 400 with wrong passphrase, got {revert_resp.status_code}"
    
    error_msg = revert_resp.json().get("detail", "")
    assert "Passphrase" in error_msg or "passphrase" in error_msg.lower() or "non è corretta" in error_msg.lower(), \
...
async def test_e2e_invalid_mapping_file():
    ...
    # Should fail with 400
    assert revert_resp.status_code == 400, \
        f"Expected 400 with invalid mapping, got {revert_resp.status_code}"
    
    error_msg = revert_resp.json().get("detail", "")
    assert "magic" in error_msg.lower() or "invalid" in error_msg.lower(), \
