...
from app.models.schemas import Finding, ReviewAction, EntityType, FindingLocation
...
def test_docx_limitation_warning_emitted(tmp_path):
    ...
    limitation_warning = any(
        "LIMITE MVP" in w and "Commenti" in w 
...
def test_xlsx_transform_preserves_formulas(tmp_path):
    ...
    assert isinstance(ws_out['B1'].value, str) and ws_out['B1'].value.startswith('='), \
