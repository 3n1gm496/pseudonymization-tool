...
from app.detectors.preprocessor import (
    canonical_ip, canonical_email, canonical_url, canonical_hostname, deobfuscate
)
...
class LDAPDNDetector(BaseDetector):
    ...
    """
    _PATTERN = re.compile(
        r'\b(?:CN|OU|DC|O|L|ST|C|UID)=[^,\s][^,]*(?:,(?:CN|OU|DC|O|L|ST|C|UID)=[^,\s][^,]*)+',
        re.IGNORECASE
    )
...
class WindowsPathDetector(BaseDetector):
    ...
    """
    _PATTERN = re.compile(
        r'\b[A-Za-z]:\\(?:[^\s<>:"/\\|?*\x00-\x1f]+\\)*[^\s<>:"/\\|?*\x00-\x1f]*'
    )
...
class MailHeadersDetector(BaseDetector):
    ...
    _HEADER_PATTERN = re.compile(
        r'^(?:From|To|Cc|Bcc|Reply-To|Sender|Return-Path|Received|Message-ID|X-Originating-IP)'
        r'\s*:\s*(.+)$',
